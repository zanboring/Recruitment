...
@Slf4j
@Component
public class Job51Parser implements JobParser {
    ...
    public List<Job> parseJobList(Document document) {
        ...
        Elements jobCards = document.select(".j_joblist");
        for (int i = 0; i < jobCards.size(); i++) {
            try {
                var card = jobCards.get(i);
                Job job = new Job();
                
                String title = card.select(".jobname").first().text();
                job.setTitle(title);
                
                String salary = card.select(".sal").first().text();
                SalaryUtil.SalaryRange range = SalaryUtil.parse(salary);
                job.setMinSalary(range.getMin());
                job.setMaxSalary(range.getMax());
                
                String company = card.select(".cname").first().text();
                ...
                log.warn("前程无忧岗位卡片解析异常: {}", e.getMessage());
            }
        }
        return jobs;
    }
